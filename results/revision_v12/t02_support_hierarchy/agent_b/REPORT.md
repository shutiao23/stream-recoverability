# REPORT — Support-hierarchy ablation for the empirical predictor (agent b)

Namespace: `results/revision_v12/t02_support_hierarchy/agent_b/`
Script: `scripts/rev_v12_t02_support_hierarchy_b.py` (run with `PYTHONPATH=$PWD/src python3 scripts/rev_v12_t02_support_hierarchy_b.py`)
All numbers below come from code run by this agent. No git, no writes outside the namespace.
...
(`second_fit_losses.csv`, 196 s wall with 8 workers).

**Verification against the frozen artifact:** all 1,446 units matched the
stored `empirical_predictions.csv`; max |Δprediction| = 8.9e-16; 100% within
...
| Reconstruction vs frozen second artifact | max diff 8.9e-16, 1,446/1,446 ✓ |
...
- `scripts/rev_v12_t02_support_hierarchy_b.py`
- `results/revision_v12/t02_support_hierarchy/agent_b/second_fit_losses.csv` (63,863 regenerated fit-loss rows)
...
Runtime: phase 1 (fit-loss regeneration) 196 s with 8 workers; full run
< 6 min on CPU only.
