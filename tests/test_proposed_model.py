...
def _inputs(batch: int = 2, steps: int = 12) -> dict[str, torch.Tensor]:
    ...
    seasonal = torch.stack(
        (torch.sin(phase), torch.cos(phase), torch.sin(phase / 2), torch.cos(phase / 2)), dim=-1
...
def test_forward_shapes_finite_values_and_ordered_quantiles() -> None:
    ...
    assert output["quantiles"].shape == (2, 12, 3, 3)
...
def test_unavailable_and_disabled_branches_have_zero_gate_weight() -> None:
    ...
    assert torch.allclose(only_a["gate_weights"][..., 0], torch.ones_like(only_a["gate_weights"][..., 0]))
...
def test_all_sixteen_information_combinations_are_supported() -> None:
    ...
    for groups in combinations:
        ...
        assert torch.isfinite(output["quantiles"]).all()
        disabled = [index for index, name in enumerate(("A", "B", "C", "D")) if name not in groups]
        if disabled:
...
def test_loss_only_uses_artificially_hidden_quality_targets() -> None:
    ...
    first = masked_imputation_loss(output, batch["target"], mask, quality_mask=batch["quality_mask"])
    changed_outside = batch["target"].clone()
    changed_outside[~mask] += 100_000
    second = masked_imputation_loss(output, changed_outside, mask, quality_mask=batch["quality_mask"])
...
def test_source_dropout_is_seeded_and_keeps_at_least_one_branch() -> None:
    first = sample_source_dropout(
        32, 0.5, generator=torch.Generator().manual_seed(22)
    )
    second = sample_source_dropout(
        32, 0.5, generator=torch.Generator().manual_seed(22)
    )
...
def test_cli_smoke_runs_on_cpu_and_saves_checkpoint(tmp_path: Path) -> None:
    ...
    assert summary["quantile_shape"][-1] == 3
    assert checkpoint.exists()
