...
def _confirmation_panel(network: str) -> pd.DataFrame:
    return read_temperature_panel(
        str(CONFIRMATION.parent / "confirmation_daily_qc/networks" / network / "daily_wide_temperature.csv")
    )
...
def _append_xgboost_roster(placements: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["season"] = pd.to_datetime(frame["gap_start"]).dt.month.map(
        lambda month: "DJF" if month in (12, 1, 2) else "MAM" if month in (3, 4, 5) else "JJA" if month in (6, 7, 8) else "SON"
    )
    return frame[
        [
            "network_id",
...
def _run_recovery_roster(
    ...
    *,
    ...
    for ordinal, network in enumerate(sorted(matched), start=1):
        print(f"development recovery roster {ordinal}/{len(matched)}: {network}", flush=True)
...
def _weighted_fit(train: pd.DataFrame, columns: list[str], outcome: str) -> np.ndarray:
    ...
    design = np.column_stack([np.ones(len(train)), train[columns].to_numpy(dtype=float)])
...
def _roster_invariance(
    ...
    for family in sorted(development_roster["model_family"].unique()):
        ...
        dev = development_predictors.drop(columns=["observed_recovery_loss"], errors="ignore").merge(
            dev_loss, on=["network_id", "station_id", "gap_length"]
        ).dropna(subset=columns)
        conf = confirmation_predictors.drop(columns=["observed_recovery_loss"], errors="ignore").merge(
            conf_loss, on=["network_id", "station_id", "gap_length"]
        ).dropna(subset=columns)
        coefficient = _weighted_fit(dev, columns, "model_loss")
        conf_prediction = coefficient[0] + conf[columns].to_numpy(dtype=float) @ coefficient[1:]
        conf = conf.assign(predicted_model_loss=conf_prediction, model_family=family)
        predictions.append(conf)
        network = conf.groupby("network_id")[["predicted_model_loss", "model_loss"]].mean()
        design = np.column_stack([np.ones(len(conf)), conf_prediction])
        weight = np.sqrt(1.0 / conf.groupby("network_id")["network_id"].transform("size").to_numpy())
        intercept, slope = np.linalg.lstsq(design * weight[:, None], conf["model_loss"].to_numpy() * weight, rcond=None)[0]
        rows.append(
            {
                "model_family": family,
                "development_networks": dev["network_id"].nunique(),
                "confirmation_networks": conf["network_id"].nunique(),
                "confirmation_station_gap_spearman": float(spearmanr(conf_prediction, conf["model_loss"]).statistic),
                "confirmation_network_spearman": float(spearmanr(network["predicted_model_loss"], network["model_loss"]).statistic),
...
def _error_model_lono(frame: pd.DataFrame, base: list[str], operator: str) -> pd.DataFrame:
    rows = []
    ...
    for held in sorted(usable["network_id"].unique()):
        ...
        for name, columns in (("learned_error_without_operator", base), ("learned_error_with_operator", [*base, operator])):
                ...
                pd.DataFrame(
                    {
                        ...
                        "observed_recovery_loss": test["observed_recovery_loss"].to_numpy(),
                    }
                )
            )
...
def _prediction_metrics(frame: pd.DataFrame, prediction: str, outcome: str) -> dict[str, float]:
    ...
    intercept, slope = np.linalg.lstsq(
        design * weight[:, None], usable[outcome].to_numpy(dtype=float) * weight, rcond=None
    )[0]
    return {
        "n": int(len(usable)),
...
def _risk_budget_curve(frame: pd.DataFrame, *, repeats: int = 100, seed: int = 0) -> pd.DataFrame:
    ...
    for domain, group in frame.groupby("domain_group"):
        ...
        for repeat in range(repeats):
            ...
            for budget in (25, 50, 100, 200):
                ...
                result = evaluate_risk_control(evaluation, rule, risk_column="predicted_loss")
                rows.append(
                    {
                        "domain_group": domain,
                        "repeat": repeat,
...
def _run_placement_replay(
    ...
    *,
    ...
    candidates = inventory.loc[
        ...
        & inventory["network_id"].isin(eligible_ids)
    ...
    return pd.concat(pairwise_parts, ignore_index=True), pd.concat(curve_parts, ignore_index=True)
...
def _plot_figures(
    ...
    stages = ["Fitting-period\ndescriptors", "Cross-network\nloss calibration", "New-network\nrank + interval", "Placement /\ntriage decision"]
    for index, label in enumerate(stages):
        axis.text(index, 0, label, ha="center", va="center", bbox={"boxstyle": "round,pad=.5", "fc": "#E8F1F8", "ec": "#355C7D"})
        if index:
            axis.annotate("", (index - 0.28, 0), (index - 0.72, 0), arrowprops={"arrowstyle": "->", "lw": 1.5})
    axis.text(0, -0.48, "gap length • memory • donor redundancy", ha="center", fontsize=9)
    axis.text(1, -0.48, "conditional variance retained as a lower bound", ha="center", fontsize=9)
    ...
    figure, (axis, residual_axis) = plt.subplots(2, 1, figsize=(7.2, 7.2), gridspec_kw={"height_ratios": [4, 1]}, sharex=True)
    for group, values in confirmation.groupby("domain_group"):
        axis.scatter(values["predicted_loss"], values["observed_recovery_loss"], s=10, alpha=0.22, color=colors[group], label=group.replace("_", " "))
    medians = confirmation.groupby("network_id")[["predicted_loss", "observed_recovery_loss"]].median()
    axis.scatter(medians["predicted_loss"], medians["observed_recovery_loss"], s=38, facecolor="white", edgecolor="black", linewidth=0.7, label="network medians")
    high = max(confirmation["predicted_loss"].max(), confirmation["observed_recovery_loss"].max())
    axis.plot([0.03, high], [0.03, high], color="black", lw=1)
    axis.set(xscale="log", yscale="log", ylabel="Realized MAE (°C)", title="Ranking transfers, while magnitude calibration shifts by domain")
    axis.legend(frameon=False, fontsize=8)
    residual_axis.scatter(confirmation["predicted_loss"], confirmation["observed_recovery_loss"] - confirmation["predicted_loss"], s=8, alpha=0.2, color="#555555")
    ...
    axis.plot(mechanism["gap_length"], mechanism["conditional_variance_lower_bound"], marker="o", label="conditional-variance lower bound")
    axis.plot(mechanism["gap_length"], mechanism["simple_prediction"], marker="s", label="simple model")
    axis.plot(mechanism["gap_length"], mechanism["realized_loss"], marker="^", label="realized loss")
    axis.set(xscale="log", xlabel="Gap length (days, log scale)", ylabel="MAE or risk scale (°C)", title="Conditional risk saturates while realized long-gap loss grows")
    axis.legend(frameon=False)
    figure.tight_layout()
    figure.savefig(output / "figure_03_mechanism.png", dpi=300)
    plt.close(figure)

    summary = replay.groupby(["policy", "protected_fraction"], as_index=False)["regret"].mean()
    figure, axis = plt.subplots(figsize=(7.2, 5.0))
    for policy, values in summary.groupby("policy"):
        if policy != "oracle":
            axis.plot(values["protected_fraction"], values["regret"], marker="o", ms=3, label=policy.replace("_", " "))
    axis.axhline(0, color="black", lw=1)
    axis.set(xlabel="Retained station fraction", ylabel="Worst-target MAE regret (°C)", title="Real-data leave-k-station-out replay")
    axis.legend(frameon=False, fontsize=7)
    figure.tight_layout()
    figure.savefig(output / "figure_04_placement_replay.png", dpi=300)
    plt.close(figure)

    budget = recalibration.groupby(["domain_group", "requested_budget"], as_index=False).agg(success=("slope_in_target_band", "mean"), slope=("evaluation_slope", "median"))
    figure, axes = plt.subplots(1, 2, figsize=(10.5, 4.4))
    for group, values in confirmation.groupby("domain_group"):
        bins = pd.qcut(values["predicted_loss"], q=min(6, values["predicted_loss"].nunique()), duplicates="drop")
        reliability = values.groupby(bins, observed=True)[["predicted_loss", "observed_recovery_loss"]].mean()
        axes[0].plot(reliability["predicted_loss"], reliability["observed_recovery_loss"], marker="o", color=colors[group], label=group.replace("_", " "))
    axes[0].plot([0, 5], [0, 5], color="black", lw=1)
    axes[0].set(xlabel="Mean predicted MAE", ylabel="Mean realized MAE", title="Domain reliability")
    axes[0].legend(frameon=False)
    for group, values in budget.groupby("domain_group"):
        axes[1].plot(values["requested_budget"], values["success"], marker="o", color=colors[group], label=group.replace("_", " "))
    axes[1].set(xlabel="Requested labelled station-gap budget", ylabel="Fraction with slope in [0.9, 1.1]", ylim=(-0.02, 1.02), title="Post-confirmation adaptation cost")
    figure.tight_layout()
...
def main() -> None:
    ...
    if not args.skip_recovery:
        ...
        dev_roster.to_csv(args.output / "development_model_roster_losses.csv", index=False)
        dev_empirical.to_csv(args.output / "development_empirical_fit_losses.csv", index=False)
        conf_roster.to_csv(args.output / "confirmation_model_roster_losses.csv", index=False)
        conf_empirical.to_csv(args.output / "confirmation_empirical_fit_losses.csv", index=False)
    else:
        ...
        conf_empirical = _read_csv(args.output / "confirmation_empirical_fit_losses.csv")

    dev_empirical_placement, dev_empirical_summary = _empirical_station_gap(dev_empirical, dev_placements)
    conf_empirical_placement, conf_empirical_summary = _empirical_station_gap(conf_empirical, conf_placements)
    dev_empirical_placement.to_csv(args.output / "development_empirical_predictions.csv", index=False)
    conf_empirical_placement.to_csv(args.output / "confirmation_empirical_predictions.csv", index=False)
    empirical_metrics = pd.DataFrame(
        [
            {"phase": "development", **_prediction_metrics(dev_empirical_summary, "empirical_transfer_prediction", "observed_recovery_loss")},
            {"phase": "confirmation", **_prediction_metrics(conf_empirical_summary, "empirical_transfer_prediction", "observed_recovery_loss")},
        ]
    )
    empirical_metrics.to_csv(args.output / "empirical_transfer_metrics.csv", index=False)
    ...
    empirical_feature = dev_empirical_summary[["network_id", "station_id", "gap_length", "empirical_transfer_prediction"]]
    learned_frame = dev_predictors.merge(empirical_feature, on=["network_id", "station_id", "gap_length"])
    learned_predictions = _error_model_lono(
        learned_frame,
        ["gap_length", "acf_only", "donor_r2_only", "additive_d_over_4_heuristic", "nearest_donor_correlation", "empirical_transfer_prediction"],
        "complete_operator_risk",
    )
    learned_predictions.to_csv(args.output / "learned_error_model_predictions.csv", index=False)
    learned_metrics = []
    for name, values in learned_predictions.groupby("model"):
        learned_metrics.append({"model": name, **_prediction_metrics(values, "prediction", "observed_recovery_loss")})
    ...
    pd.DataFrame([decomposition]).to_csv(args.output / "rank_decomposition.csv", index=False)
    ...
    empirical_risk_frame = conf_empirical_summary.dropna(
        subset=["empirical_transfer_prediction"]
    ).rename(
    ...
    empirical_block = network_block_scaled_intervals(
        dev_empirical_summary.dropna(subset=["empirical_transfer_prediction"]),
        conf_empirical_summary.dropna(
            subset=["empirical_transfer_prediction"]
        ).rename(
            columns={"empirical_transfer_prediction": "predicted_loss"}
        ),
    ...
    recalibration_summary = (
        recalibration.groupby(["domain_group", "requested_budget"])["slope_in_target_band"]
        .mean()
    ...
    risk_summary = (
        ...
        .agg(
            certified_fraction=("status", lambda values: float(np.mean(values.eq("certified")))),
    ...
    summary = {
        ...
        "placement_mean_regret": {str(key): float(value) for key, value in replay_summary.items()},
