# Case study v4 lineage (canonical)

> **Canonical copy:** [`paper/case_study_v1/manuscript.md`](case_study_v1/manuscript.md)  
> **Design:** `design_freeze_v4` · See [`paper/study_manifest.json`](study_manifest.json)

# A Case-Study Covariance Heuristic for Stream-Temperature Recoverability in Two Regulated River Networks

## Abstract

Monitoring outages can remove weeks to months of stream-temperature records, yet the recoverability of a gap depends on which temporal and network information remains. We evaluated a train-only covariance heuristic across three Upper Jinsha River stations and five stations along the Upper--Middle Chattahoochee River using controlled 30--180-day gaps and a fixed roster of conventional recovery models. The heuristic separated sites whose recovery relied primarily on local boundary memory from sites benefiting from synchronous donor stations and reproduced the shape of internal best-model skill curves. It did not, however, provide transferable magnitude calibration: at the dam-proximal Chattahoochee site, the predicted 180-day skill was 0.414 whereas the post-hoc fixed-model skill was $-0.300$. An independently frozen 335-site United States panel also failed to support the heuristic as a standalone indicator of upstream major dams. These results identify covariance diagnostics as a useful local audit of monitoring redundancy, but not as a general reservoir classifier or a validated rule for gap filling.
...
Across two regulated river networks, fitting-period covariance structure distinguished sites relying primarily on local thermal memory from sites benefiting from synchronous network observations. This distinction was useful as a local diagnostic but did not transfer as a calibrated recovery rule, and an independently frozen national panel did not support the index as a standalone dam classifier. The evidence therefore supports using covariance diagnostics to audit monitoring redundancy within a calibrated network, not to infer reservoir causation, remove sensors, or certify long-gap reconstruction.

## 6. Data and Code Availability

The analysed Jinsha daily hydrological records were supplied to the project and are attributed to the Chinese Hydrological Yearbook. Permission to redistribute the exact $T$, $F$, and $L$ files was not established. CMA humidity and sunshine and the WMO/CMA meteorological series are likewise restricted under the rights matrix. Subject to written editor approval of this restricted third-party-data exception, the exact analysis inputs will be provided through the journal's confidential peer-review workflow; they are not “available upon request.” The paper will not be submitted without that approval. Public aggregate tables and figures do not redistribute daily records.
