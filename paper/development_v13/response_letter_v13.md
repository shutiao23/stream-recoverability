# Response letter v13 (merged, adversarial pair): point-by-point response to the simulated WRR review
...
(`manuscript_v13.md`, `figure_plan_v13.md`, `claim_matrix_v13.md`,
`terminology_v13.md`, `protocol_v4.md`, `response_letter_v13.md`,
`evidence_ledger_v13.md`). All numbers cited below are artifact values from
...
2. **Protocol v3 → v4 (replacement, `protocol_v4.md`).** v4 replaces
...
| --- | --- |
| MC1: strongest-baseline comparator in abstract/Fig 2 | `manuscript_v13.md` §1 (abstract), §3.1; Fig 2; ledger rows S1–S2 |
| MC2: second panel is post-hoc development; provenance labels | §2.10 evidence roles; abstract provenance sentence; Fig 1 legend; `terminology_v13.md`; ledger Evidence-role column |
...
| MC4: coverage floors for model selection | §3.8; Fig 4; `protocol_v4.md` §4 (coverage floor + regret endpoint); ledger rows D4–D5 |
| MC5: downstream multi-baseline (no-fill vs climatology) | §3.7; Fig 5; SI downstream tables (both defaults); ledger rows F1–F5 |
| MC6: BiLSTM relabeled cross-instance; common-panel required | §3.4 (SI); terminology; SI model matrix with n; `protocol_v4.md` §5 (per-model self-transfer on identical gaps); ledger rows M1–M3 |
...
| MC8: surface interpolation vs extrapolation; 365 abstention | §3.2; Fig 3; `protocol_v4.md` §7 (T3 exclusion); ledger rows U1–U6 |
| MC9: persistence law; Design as framework | §2.3 (framework definition), §3.1–3.3, §5 (conclusions); ledger rows C1–C7 |
| MC10: downstream-centered significance, multiple baselines | §3.7, §3.8 reordered; Fig 5; `protocol_v4.md` endpoint (d) floor; ledger rows F1–F5 |
| Writing: title/abstract/key points/plain language/intro/results order/conclusions | `manuscript_v13.md` (title kept, abstract <250 words, key points ≤140 chars, 5-paragraph intro, results reordered, conclusions without "strongest screen") |
| Figures: Fig1 provenance; Fig2 strongest baseline; Fig3 regimes; Fig4 coverage–regret; Fig5 multi-baseline; matrices to SI | `figure_plan_v13.md` (Figs 1–5; SI displays S6–S7 with n-annotations) |
| Data/software: archival DOI before submission | `manuscript_v13.md` Open Research (date-gated checklist; no placeholder DOI) |
| Data/software: protocol v3→v4 | `protocol_v4.md` (regret endpoint, coverage floor, T3 abstention, external timestamping) |

## Open items carried into v13 (see also the evidence ledger TODO rows)

- Archival DOI (mandatory before submission; date-gated deposit workflow defined in Open Research and the submission checklist).
...
- t07 convention divergence (canonical subset-20 values used; the independent re-run agrees qualitatively).
- t06 harmonization (matrix demoted; a harmonized re-run on one 12-network panel with one forcing definition is required before any revival of the 0.979/0.490 numbers).
- Figure renderings from the listed source CSVs; length check (trim Methods/SI if the journal limit binds).
- Protocol v4 external registration (pre-outcome commit + OSF/Zenodo) before any third-panel outcome is opened.
