...
`paper/development_v13/aux_text_v13.md` (this file's companion).
...
folder; consolidated at `paper/development_v13/protocol_v4.md`)
...
rank-magnitude superiority over this baseline. Per horizon, the
empirical curve's edge over r6 is concentrated at long gaps: network
Spearman was 0.932/0.916/0.865/0.659 for the empirical predictor versus
0.938/0.915/0.843/0.603 for r6 at 7/30/90/180 days, so r6 was equal or
slightly better at 7 and 30 days and the empirical curve gained at 90 and
180 days. The increment over the
...
level. On the first panel (direct_858 units, 42
...
different networks, only 5 shared with implementation A, including Czech,
Dutch, and US networks; different forcing definitions) found matched
donor-synchronous transfer of only 0.490, with uniform-block matched
transfer 0.944 (versus 0.531 in implementation A--a second large
conflict) and a much milder uniform-on-donor-synchronous mismatch (0.524).
The two implementations
differ in their 12-network panels (verified: 5 networks in common) and in
the forcing definition (target
...
are unified, no mechanism-matched transfer value--including
implementation A's donor-synchronous 0.979--is asserted as evidence
anywhere in this manuscript; the
...
## Editorial note (not part of the manuscript): review-text versus artifact discrepancies
